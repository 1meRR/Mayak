...
use domain::models::{
    ...
    CallInviteListResponse, CreateCallInviteRequest, CreateFriendRequestRequest, ErrorResponse,
    LoginRequest, PendingMessagesQuery, PendingMessagesResponse, RegisterRequest,
    RespondCallInviteRequest, RespondFriendRequestRequest, SendEncryptedMessageRequest,
    SendEncryptedMessageResponse, UpsertDeviceKeyPackageRequest, UserLookupResponse,
};
...
async fn create_call_invite(
    ...
    match state
        .store
        .create_call_invite(
            &req.caller_public_id,
            &req.caller_device_id,
            &req.session_token,
            &req.callee_public_id,
        )
        .await
    {
...
async fn get_incoming_calls(
    ...
    match state.store.list_incoming_call_invites(&public_id).await {
        Ok(items) => (StatusCode::OK, Json(CallInviteListResponse { items })).into_response(),
        Err(err) => error_response(StatusCode::BAD_REQUEST, err),
    }
}
